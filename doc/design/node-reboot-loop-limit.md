...
flowchart TD
    ...
    H --> I{Reboot required?}
    I -->|Yes| I2{Boot ID matches\nstored boot ID?}
    I2 -->|Yes| I3[Skip reboot, requeue]
    I2 -->|No| J[Increment counter\nUpdate boot ID\nWrite to disk]
    J --> K[Reboot node]
...
* `incrementRebootCounter(generation int64) error` — reads the current boot ID from
  `/proc/sys/kernel/random/boot_id`. If the tracker already has the same boot ID
  (reboot already requested this boot), skips the increment. Otherwise increments
  the counter (or creates a new tracker for a new generation), updates the boot ID,
  and writes the tracker. Called before `rebootNode()`
